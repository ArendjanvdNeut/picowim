...
Function  AppendFile( textfile, extraRow )
	...
	if fs.FileExists( textfile ) then
		...
		'result="append failed: no file"
		result = WriteFile( textfile, content )
	end if
	set fs=nothing
	Appendfile = result
end function
...
Function  WriteFile( textfile, content )
	...
	If Err.Number <> 0 Then	
		result = "Write failed: ("&Err.Number&")" & Err.Description
...
Function  ListFolder( foldername )
	...
	Set objFolder = objFSO.GetFolder(foldername) 
	'Loop through the Files collection 
	Dim objFile, result,thisfile,comma,rows
	comma = ""
	result="["
	For Each objFile in objFolder.Files
		result=result+comma+"{""file"":"+""""+objFile.name+""","
		comma=","
		rows = CountFile( objFile )
		result=result&"""rows"":"""&rows&""","
		result=result&"""date"":"&""""&objFile.DateLastModified&"""}"
	Next 
	result = result+"]"
